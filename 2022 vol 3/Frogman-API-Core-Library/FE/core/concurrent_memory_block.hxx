...
{
private:
	concurrency::IN_USE m_is_being_used;
	FE::atomic_volatile<var::boolean> m_is_block_constructed;
	T m_object;
...
public: 
	...
	{
		if (FE::exception::handle_exceptions(this->m_is_block_constructed.load(std::memory_order_acquire) == false, FE::_ABORT_IMMEDIATELY_, "WARNING: failed to access the memory block because the block is not constructed. The function will return early with a null object.", _EXCEPTION_ORIGIN_))
#if _HAS_CXX20_ == 1
			_UNLIKELY_
#endif
		{
