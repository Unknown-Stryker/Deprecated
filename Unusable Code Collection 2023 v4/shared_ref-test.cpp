...
//
//TEST(shared_ref, serial)
//{
//	FE::concurrent_memory_block<my_struct> l_concurrent_memory_block;
//
//	{
//		l_concurrent_memory_block.construct();
//		EXPECT_TRUE((**l_concurrent_memory_block)._has_constructor_been_called);
//
//		FE::concurrency::shared_ref<my_struct> l_movable_scoped_ref = l_concurrent_memory_block; // Critical Section. Threads can not invoke destruct() of l_concurrent_memory_block from this point
//		
//		EXPECT_TRUE(l_movable_scoped_ref.is_being_used());
//
//		(*l_movable_scoped_ref)._field_variable = 5; // Critical Section.
//
//		if (l_concurrent_memory_block.destruct() == FE::_FAILED_) // It always fail to invoke the destructor, since l_concurrent_memory_block is being referenced by l_movable_scoped_ref.
//		{
...
//
//	l_concurrent_memory_block.destruct();
//
...
//{
//	FE::concurrency::shared_ref<my_struct> l_movable_scoped_reference = concurrent_memory_block_p;
//	promise_p.set_value(true);
//	std::this_thread::sleep_for(std::chrono::milliseconds(10));
//    EXPECT_EQ(l_movable_scoped_reference.is_being_used(), true);
//}
//
//TEST(shared_ref, parallel)
//{
...
//	l_function._arguments_buffer._second.construct();
...
//
//	while (l_function._arguments_buffer._second.destruct() == false) {} // Busy waiting l_worker's job to be done 
//
