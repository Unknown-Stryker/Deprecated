...
#define FRIEND_CLASS_CONCURRENT_SINGLETON(typename_p) friend class ::FE::memory_block<typename_p>; friend class ::FE::concurrent_singleton<typename_p>
...
{
private:
	static FE::memory_block<T> s_instance;
...
public: 
	...
	_FORCE_INLINE_ static T& singleton_instance() noexcept
	{
		...
		{
			concurrent_singleton<T>::s_instance.construct();
		}
	...
	_FORCE_INLINE_ static T& singleton_instance(Arguments&& ...arguments_p) noexcept
	{
		if (concurrent_singleton<T>::s_instance.is_constructed() == false) _UNLIKELY_
		{
			concurrent_singleton<T>::s_instance.construct(arguments_p...);
		}
	...
	{
		concurrent_singleton<T>::s_instance.destruct();
	}
};

template<typename T>
::FE::memory_block<T> FE::concurrent_singleton<T>::s_instance;
