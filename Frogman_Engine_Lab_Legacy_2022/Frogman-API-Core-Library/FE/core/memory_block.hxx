...
public:
	...
	{
		if (FE::exception::handle_exceptions(this->m_is_block_constructed == false, FE::_LOG_EXCEPTION_HISTORY_, "WARNING: failed to dereference the memory block because the block is not constructed. The function will return early with a null object.", _EXCEPTION_ORIGIN_))
#if _HAS_CXX20_ == 1
			_UNLIKELY_
#endif
		{
	...
	{
		if (FE::exception::handle_exceptions(this->m_is_block_constructed == false, FE::_LOG_EXCEPTION_HISTORY_, "WARNING: failed to access the memory block because the block is not constructed. The function will return early with a null object.", _EXCEPTION_ORIGIN_))
#if _HAS_CXX20_ == 1
			_UNLIKELY_
#endif
		{
