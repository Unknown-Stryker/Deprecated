...
{
    FE::exception::handle_exceptions(this->m_is_locked.load(std::memory_order_acquire) == true, FE::_ABORT_IMMEDIATELY_, "CRITICAL ERROR: mutex is still locked", _EXCEPTION_ORIGIN_);
...
{
    FE::exception::handle_exceptions(this->m_is_locked.load(std::memory_order_acquire) == true, FE::_ABORT_IMMEDIATELY_, "CRITICAL ERROR: timed mutex is still locked", _EXCEPTION_ORIGIN_);
